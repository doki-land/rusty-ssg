...
use crate::{
    BuildArgs,
    jekyll::{JekyllConfig, JekyllConfigLoader, JekyllStructure, PostManager},
    types::Result,
};
use console::style;
...
impl BuildCommand {
    ...
    pub async fn execute(args: BuildArgs) -> Result<()> {
        ...
        println!("  {} Loading directory structure...", style("→").blue());
        let structure = JekyllStructure::new(&source_dir)?;
        println!("  {} Directory structure loaded", style("✓").green());
