        ...
        while i < lines.len() {
            ...
            if line.starts_with("```") {
                ...
                result.push_str(&format!(
                    ...
                    lang, self.escape_html(&code_content)
                ));
                continue;
            }
            ...
            if line.starts_with('*') || line.starts_with('-') || line.starts_with('+') {
                // 处理无序列表
                result.push_str("<ul>\n");
                while i < lines.len() && (lines[i].trim().starts_with('*') || lines[i].trim().starts_with('-') || lines[i].trim().starts_with('+')) {
            ...
            // 处理表格
            if line.contains('|') && lines.get(i + 1).map_or(false, |l| l.contains('|') && l.chars().all(|c| c == '|' || c == '-' || c.is_whitespace())) {
