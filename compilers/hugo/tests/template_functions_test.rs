...
fn test_seq() {
    ...
    assert_eq!(
        funcs.seq(&[json!(5)]).unwrap(),
        json!([1, 2, 3, 4, 5])
    );
    
    assert_eq!(
        funcs.seq(&[json!(10), json!(5), json!(2)]).unwrap(),
        json!([5, 7, 9])
    );
}
...
fn test_union() {
    ...
    assert_eq!(
        funcs.union(&[json!([1, 2, 3]), json!([3, 4, 5])]).unwrap(),
        json!([1, 2, 3, 4, 5])
    );
}
...
fn test_intersection() {
    ...
    assert_eq!(
        funcs.intersection(&[json!([1, 2, 3]), json!([3, 4, 5])]).unwrap(),
        json!([3])
    );
}
...
fn test_difference() {
    ...
    assert_eq!(
        funcs.difference(&[json!([1, 2, 3, 4]), json!([3, 4, 5])]).unwrap(),
        json!([1, 2])
    );
}
...
fn test_relref() {
    let funcs = PageFunctions::new();
    assert_eq!(
        funcs.relref(&[json!("about")]).unwrap(),
        json!("/about")
    );
}
...
fn test_truncate() {
    ...
    assert_eq!(
        funcs.truncate(&[json!(5), json!("Hello World")]).unwrap(),
        json!("Hello...")
    );
    
    assert_eq!(
        funcs.truncate(&[json!(5), json!("Hello World"), json!("..")]).unwrap(),
        json!("Hello..")
    );
}
