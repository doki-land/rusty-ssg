...
use hugo::types::{
    document::hugo_content::{HugoFrontMatter, HugoPage},
    taxonomies::{Taxonomy, TaxonomyBuilder, TaxonomyIndex, TaxonomyTerm},
};
...
fn test_build_from_pages() {
    let page1 = create_test_page("First Post", vec!["rust".to_string(), "web".to_string()], vec!["programming".to_string()]);
...
fn test_slugify() {
    ...
    let page = create_test_page("Test", vec!["Hello World".to_string(), "Rust!".to_string(), "Test 123".to_string()], vec![]);
