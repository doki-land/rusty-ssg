...
impl PluginHostError {
    ...
    pub fn i18n_key(&self) -> &'static str {
        match self {
            PluginHostError::ProcessStartError { .. } => "hugo.error.plugin_host.process_start",
            PluginHostError::IoError { .. } => "hugo.error.plugin_host.io",
            PluginHostError::JsonError { .. } => "hugo.error.plugin_host.json",
            PluginHostError::Timeout { .. } => "hugo.error.plugin_host.timeout",
            PluginHostError::ProcessShutdown => "hugo.error.plugin_host.process_shutdown",
            PluginHostError::InvalidResponse => "hugo.error.plugin_host.invalid_response",
        }
    }
...
impl PluginHost {
    ...
    pub fn shutdown(&mut self) -> Result<()> {
        ...
        if let Some(mut process) = self.process.take() {
            ...
            process.kill()?;
            let _ = process.wait().map_err(|e| PluginHostError::IoError { source: e })?;
        }
