...
impl From<toml::de::Error> for VutexError {
...
impl From<std::net::AddrParseError> for VutexError {
...
impl From<WaeError> for VutexError {
    fn from(e: WaeError) -> Self {
        VutexError::IoError { message: e.to_string() }
    }
}
...
impl VutexError {
    ...
    pub fn i18n_key(&self) -> &'static str {
        match self {
            VutexError::ParseError { .. } => "hugo.error.parse",
            VutexError::DocumentNotFound { .. } => "hugo.error.document_not_found",
            VutexError::ConfigError { .. } => "hugo.error.config",
            VutexError::IoError { .. } => "hugo.error.io",
            VutexError::External { .. } => "hugo.error.external",
        }
    }
