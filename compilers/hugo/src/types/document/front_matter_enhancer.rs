...
impl FrontMatterEnhancer {
    ...
    pub fn validate(frontmatter: &HugoFrontMatter) -> Result<(), HugoContentError> {
        ...
        if let Some(date) = &frontmatter.date {
            if !Self::is_valid_date(date) {
                return Err(HugoContentError::front_matter_parse_error(format!("Invalid date format: {}", date)));
            }
        }

        if let Some(lastmod) = &frontmatter.lastmod {
            if !Self::is_valid_date(lastmod) {
                return Err(HugoContentError::front_matter_parse_error(format!("Invalid lastmod format: {}", lastmod)));
            }
        }

        if let Some(publish_date) = &frontmatter.publish_date {
            if !Self::is_valid_date(publish_date) {
                return Err(HugoContentError::front_matter_parse_error(format!(
                    "Invalid publishDate format: {}",
                    publish_date
                )));
            }
        }
        ...
        if let Some(expiry_date) = &frontmatter.expiry_date {
            if !Self::is_valid_date(expiry_date) {
                return Err(HugoContentError::front_matter_parse_error(format!("Invalid expiryDate format: {}", expiry_date)));
            }
        }
    ...
    pub fn process_resources(frontmatter: &HugoFrontMatter) -> Vec<String> {
        frontmatter
            .resources
            .as_ref()
            .map(|resources| resources.iter().filter_map(|res| res.name.clone()).collect())
            .unwrap_or_default()
    }
}
