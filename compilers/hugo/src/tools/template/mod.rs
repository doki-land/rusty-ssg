...
impl UnifiedTemplateManager {
    /// 创建新的统一模板管理器
    pub fn new() -> Self {
        Self { manager: TemplateManager::new() }
    }

    /// 注册模板
    pub fn register_template(&mut self, engine: TemplateEngine, name: &str, content: &str) -> Result<(), std::io::Error> {
        self.manager.register_template(engine, name, content)
    }

    /// 注册模板文件
    pub fn register_template_file(&mut self, engine: TemplateEngine, name: &str, path: &Path) -> Result<(), std::io::Error> {
        self.manager.register_template_file(engine, name, path)
    }

    /// 从目录注册模板
    pub fn register_templates_from_dir(&mut self, engine: TemplateEngine, dir: &Path) -> Result<(), std::io::Error> {
    ...
    pub fn render_json(&self, engine: TemplateEngine, template_name: &str, context: &Value) -> Result<String, std::io::Error> {
