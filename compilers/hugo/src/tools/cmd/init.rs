...
impl InitCommand {
    ...
    pub async fn execute(args: InitArgs) -> Result<()> {
        println!("{}", style("Initializing VuTeX project...").cyan());

        let project_name = args.name.unwrap_or_else(|| "vutex-docs".to_string());
        ...
        println!("{}", style("✓ VuTeX project initialized successfully!").green());
        println!("\nNext steps:");
        println!("  1. {} - Start development server", style("vutex dev").cyan());
        println!("  2. {} - Build static site", style("vutex build").cyan());
    ...
    fn create_directory_structure() -> Result<()> {
        let dirs = [".vutex", "guide", "public/images"];
...
"#,
        ...
        fs::write(".vutex/vutex.config.toml", config_content)?;
        println!("  {} Created config file: .vutex/vutex.config.toml", style("✓").green());
    ...
    fn create_index_document() -> Result<()> {
        let index_content = r#"# 欢迎使用 VuTeX

VuTeX 是一个高性能的文档生成系统，基于 Rust 构建，速度飞快！

## 快速开始

1. 编辑 `index.md` 开始你的文档
2. 运行 `vutex dev` 启动开发服务器
...
欢迎来到 VuTeX 指南！这里将教你如何使用 VuTeX 构建出色的文档站点。
...
VuTeX 已经安装在你的系统中，可以直接使用。

## 创建项目

使用 `vutex init` 命令可以快速创建一个新的 VuTeX 项目。

## 开发模式

运行 `vutex dev` 启动开发服务器，然后在浏览器中访问 http://localhost:5173。

## 构建

运行 `vutex build` 生成静态站点文件。
"#;
