...
impl PluginCommand {
    ...
    pub async fn execute(args: PluginArgs) -> crate::types::Result<()> {
        match args.subcommand {
            PluginSubCommand::Install(install_args) => Self::install(install_args).await,
            PluginSubCommand::Uninstall(uninstall_args) => Self::uninstall(uninstall_args).await,
            PluginSubCommand::List => Self::list().await,
        }
    }
