use gatsby::{
    ContentDigest, GraphQLError, GraphQLExecutor, GraphQLRequest, GraphQLResult, Node, NodeBuilder, NodeId, NodeStore,
    NodeType, SchemaBuilder,
};
...
fn test_node_builder() {
    ...
    let result = NodeBuilder::new().id(id.clone()).type_name(type_name.clone()).content_digest(digest).build();
...
fn test_node_builder_missing_id() {
    ...
    let result = NodeBuilder::new().type_name(type_name).content_digest(digest).build();
...
fn test_schema_builder() {
    let result = SchemaBuilder::new().query_type("Query".to_string()).build();
...
fn test_graphql_executor_creation() {
    let schema = SchemaBuilder::new().query_type("Query".to_string()).build().unwrap();
...
fn test_graphql_request_execution() {
    ...
    let request = GraphQLRequest { query: "{ allNodes { id } }".to_string(), operation_name: None, variables: None };
