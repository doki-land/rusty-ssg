...
impl DataSystem {
    ...
    fn load_data_file<P: AsRef<Path>>(&self, path: P) -> Result<serde_json::Value, DataError> {
        ...
        match extension.as_str() {
            "json" => serde_json::from_str(&content).map_err(|e| DataError::ParseError(e.to_string())),
            "yaml" | "yml" => serde_yaml::from_str(&content).map_err(|e| DataError::ParseError(e.to_string())),
            "toml" => toml::from_str(&content).map_err(|e| DataError::ParseError(e.to_string())),
            _ => Err(DataError::LoadError(format!("Unsupported file type: {}", extension))),
        }
    }
