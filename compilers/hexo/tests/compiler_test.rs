...
use hexo::compiler::{Compiler, Parser, Renderer};
...
"#;
    ...
    assert_eq!(parsed["date"], "2024-01-01 10:00:00");
    assert!(parsed["categories"].as_array().unwrap().contains(&serde_json::Value::String("test".to_string())));
    assert!(parsed["tags"].as_array().unwrap().contains(&serde_json::Value::String("hexo".to_string())));
